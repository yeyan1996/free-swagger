...
const compileInterface = (source, interfaceName) => {
    if (!source.definitions)
        return '';
    return source.definitions[interfaceName] && !interface_2.shouldSkipGenerate(interfaceName)
        ? fp_1.pipe(fp_1.curry(interface_2.parseInterface)(fp_1.curry.placeholder, interfaceName, false), interface_1.genInterface, utils_1.formatCode('ts'))(source.definitions)
        : '';
};
const compileInterfaces = (source, interfaceName) => {
    ...
    if (interfaceName) {
        ...
        const interfaceCode = Object.keys(source.definitions).reduce((acc, cur) => acc + compileInterface(source, cur), '');
        const interfaceWithGenericCode = Object.keys(interface_2.recursiveMap).reduce((acc, cur) => acc + utils_1.formatCode('ts')(interface_1.genInterface(interface_2.recursiveMap[cur])), '');
